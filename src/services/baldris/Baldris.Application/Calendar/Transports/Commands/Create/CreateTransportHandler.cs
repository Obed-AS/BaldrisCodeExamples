...
{
    internal class CreateTransportHandler : IRequestHandler<CreateTransportCommand, TransportDto>
    {
        ...
        public CreateTransportHandler(IBaldrisDbContextFactory dbContextFactory, IMapper mapper,
        ...
        public async Task<TransportDto> Handle(CreateTransportCommand request, CancellationToken cancellationToken)
        {
            ...
            var entity = _mapper.Map<CreateTransportCommand, Transport>(request);

            await dbContext.Transports.AddAsync(entity, cancellationToken);

            var userId = await _loggedInUserService.GetUserIdAsync();
            await dbContext.SaveChangesAsync(userId, cancellationToken);

            // Send Notification
            await _mediator.Publish(new EntityCreatedNotification<Transport>
                { CreatedId = entity.Id, CreatedValue = entity, UserId = userId }, cancellationToken);

            // Get newly inserted entity
            var insertedEntity = await dbContext.Transports
