...
{
    public class UpdateOrInsertGroomingCommand : IRequest<GroomingDto>, IHaveCustomMapping
    {
        ...
        {
            configuration.CreateMap<UpdateOrInsertGroomingCommand, Grooming>()
                .ForMember(target => target.Id, opt => opt.MapFrom(source => source.Id ?? Guid.Empty));
            configuration.CreateMap<UpdateOrInsertGroomingCommand, UpdateGroomingCommand>().ReverseMap();
            configuration.CreateMap<UpdateOrInsertGroomingCommand, CreateGroomingCommand>().ReverseMap();
        }
    }
}
