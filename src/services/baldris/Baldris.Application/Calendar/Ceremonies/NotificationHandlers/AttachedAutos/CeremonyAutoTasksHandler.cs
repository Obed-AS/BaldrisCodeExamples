...
{
    internal class CeremonyAutoTasksHandler : INotificationHandler<EntityCreatedNotification<Ceremony>>,
        ...
        INotificationHandler<EntityDeletedNotification<Ceremony>>
    {
        ...
        public CeremonyAutoTasksHandler(IBaldrisDbContextFactory dbContextFactory, ITenantService tenantService, IMediator mediator)
        {
                ...
                if (notification.CreatedValue.CalendarEventTypeId.HasValue)
                {
                    await _mediator.Send(new CreateAutoTasksByAttachedTypeIdCommand<CalendarEventType>(
                        ...
                        nameof(CalendarEvent), notification.GoverningId.Value,
                         values.DepartmentId, Discriminator.Ceremony, GetNoteOrTaskTab(notification.CreatedValue), null, new AutoTaskReplacements
                        {
                ...
                if (notification.PreviousValue.CalendarEventTypeId != notification.CurrentValue.CalendarEventTypeId)
                {
                    ...
                    if (notification.CurrentValue.CalendarEventTypeId.HasValue)
                    {
                        await _mediator.Send(new CreateAutoTasksByAttachedTypeIdCommand<CalendarEventType>(
                            ...
                            nameof(CalendarEvent), notification.GoverningId.Value,
                            values.DepartmentId, Discriminator.Ceremony, GetNoteOrTaskTab(notification.CurrentValue), null, new AutoTaskReplacements
                            {
                                DeceasedNames = values.Deceased,
                                Value = values.Value,
                                LocationName = values.Location
